...
def test_when_print_info_runs_it_outputs_metadata(capsys: pytest.CaptureFixture[str]) -> None:
    ...
    from proxmox_srvbackup import print_info

    print_info()

    captured = capsys.readouterr().out
    assert "proxmox_srvbackup" in captured
...
def test_metadata_constants_are_set() -> None:
    ...
    assert __init__conf__.name == "proxmox_srvbackup"
    assert __init__conf__.version  # Should have a version
    assert __init__conf__.shell_command == "proxmox-srvbackup"
